...
#    pass
with open('fileLists/100417_AODSIMfiles.txt') as freco:
...
for i,iFile in enumerate(fileList):
    #i = int(n)
	print iFile.split('/')[7].split('AODSIM.')[0] ##
	inputFile = 'file:'+iFile
	outputFile = 'file:/eos/user/k/kmei/SeptGenSplitSUSY/'+str(iFile.split('/')[7].split('AODSIM.')[0])+'MINIAOD.root'
	outF = open("sh/runSplitSUSY_MINIAOD_" + str(i) + ".sh" , 'w')
	outF.write("#!/bin/bash\n")
	outF.write("\n")
	...
	outF.write('export CFG_FILE=$CMSSW_PROJECT_SRC/AODSIMtoMINIAODcfg_TEMPLATE.py\n')
	...
	outF.write("\n")
	outF.close()
	os.system("bsub -q 8nh -J SplitSUSY_MINIAOD_" + str(i) + " < sh/runSplitSUSY_MINIAOD_" + str(i) + ".sh")
