...
#    pass
with open('fileLists/test_RECO.txt') as freco:
...
for i,iFile in enumerate(fileList):
	print str(iFile.split('/')[7].split('RECO')[0])+"AODSIM"+str(iFile.split('/')[7].split('RECO')[1])
	inputFile = 'file:'+iFile
	outputFile = 'file:/eos/user/k/kmei/OctGenSplitSUSY/AODSIM/'+str(iFile.split('/')[7].split('RECO')[0])+'AODSIM'+str(iFile.split('/')[7].split('RECO')[1])
	outF = open("sh/runSplitSUSY_AODSIM_" + str(i) + ".sh" , 'w')
	outF.write("#!/bin/bash\n")
	outF.write("\n")
	outF.write('export BASE_DIR=/afs/cern.ch/work/k/kmei/TauGeneration\n')
	outF.write('export CMSSW_PROJECT_SRC=$BASE_DIR/CMSSW_8_0_21/src\n')
    #outF.write('export JOB_DIR=$BASE_DIR/%s/%s\n'%(tName,pName))
	outF.write('export CFG_FILE=$CMSSW_PROJECT_SRC/RECOtoAODSIMcfg_TEMPLATE.py\n')
	...
	outF.close()
	#print outputFile
	#os.system("bsub -q 2nd -J SplitSUSY_AODSIM_" + str(i) + " < sh/runSplitSUSY_AODSIM_" + str(i) + ".sh")
